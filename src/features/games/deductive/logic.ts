...
function generateLatinSquare(size: number, symbols: GameSymbol[]): (GameSymbol | null)[][] {
    const grid: (GameSymbol | null)[][] = Array.from({ length: size }, () => Array(size).fill(null));
    function isSafe(row: number, col: number, symbol: GameSymbol): boolean {
...
function isObviouslyWrong(symbol: GameSymbol, grid: (GameSymbol | null)[][], cell: { row: number; col: number }) {
...
export function checkAnswer(puzzle: Puzzle, symbol: GameSymbol): boolean {
